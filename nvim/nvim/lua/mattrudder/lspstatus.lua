...
local function get_lsp_progress()
  ...
  for _, msg in ipairs(buf_messages) do
    ...
    if msg.progress then
      ...
      if msg.percentage then contents = string.format("(%.0f%%%%) ", msg.percentage) .. contents end
    ...
    table.insert(msgs, vim.trim(contents))
  end
  return vim.trim(table.concat(msgs, config.component_separator))
end

local function my_lsp_status(bufnr)
  bufnr = bufnr or 0
